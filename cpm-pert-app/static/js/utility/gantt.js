...
function renderGantt(result) {
  ...
  const projectDuration = Math.max(1, ...items.map(t => Math.max(t.lf || 0, t.ef || 0)));
  ...
  items.forEach((item, i) => {
    const ghost = ganttGhostData ? ganttGhostData.find(g => g.id === item.id) : null;
...
function buildGanttHeader(svg, projectDuration, svgH, svgW, projectStart) {
  svg.appendChild(makeSvgEl("rect", { x: 0, y: 0, width: svgW, height: GANTT_HDR_H, fill: "#f1f5f9" }));

  const hdrLbl = makeSvgEl("text", {
    x: GANTT_LBL_W / 2, y: GANTT_HDR_H / 2,
  ...
  svg.appendChild(makeSvgEl("line", {
    x1: GANTT_LBL_W, y1: 0, x2: GANTT_LBL_W, y2: svgH, stroke: "#94a3b8", "stroke-width": 1,
  }));

  for (let d = 0; d <= projectDuration + 1; d++) {
    const x = ganttX(d);
    svg.appendChild(makeSvgEl("line", {
      x1: x, y1: GANTT_HDR_H, x2: x, y2: svgH,
    }, ["gantt-grid-line"]));

    const dateStr = toISODate(addDays(projectStart, d)).slice(5); // MM-DD
    const dLbl = makeSvgEl("text", {
      x: x + GANTT_COL_W / 2, y: GANTT_HDR_H * 0.32,
      "text-anchor": "middle", "dominant-baseline": "middle",
    }, ["gantt-header-text"]);
    dLbl.textContent = dateStr;
    svg.appendChild(dLbl);

    const uLbl = makeSvgEl("text", {
      x: x + GANTT_COL_W / 2, y: GANTT_HDR_H * 0.72,
      "text-anchor": "middle", "dominant-baseline": "middle",
    }, ["gantt-header-unit"]);
    uLbl.textContent = d;
  ...
  svg.appendChild(makeSvgEl("line", {
    x1: 0, y1: GANTT_HDR_H, x2: svgW, y2: GANTT_HDR_H, stroke: "#94a3b8", "stroke-width": 1.5,
  }));
}
...
function buildGanttRow(svg, item, ghostItem, rowIndex, svgW) {
  ...
  g.appendChild(makeSvgEl("rect", {
    x: 0, y, width: svgW, height: GANTT_ROW_H,
    fill: rowIndex % 2 === 0 ? "#ffffff" : "#f8fafc",
  }));

  const rowLbl = makeSvgEl("text", {
    x: GANTT_LBL_W - 8, y: y + GANTT_ROW_H / 2,
  ...
  g.appendChild(makeSvgEl("line", {
    x1: GANTT_LBL_W, y1: y, x2: GANTT_LBL_W, y2: y + GANTT_ROW_H,
  }, ["gantt-grid-line"]));
  ...
  if (ghostItem && ghostItem.ef !== item.ef) {
    ...
    g.appendChild(makeSvgEl("rect", { x: gx, y: barY, width: gw, height: GANTT_BAR_H, rx: 4 }, ["gantt-ghost"]));
  }
  ...
  if (item.lf > item.ef) {
    ...
    g.appendChild(makeSvgEl("rect", { x: fx, y: barY, width: fw, height: GANTT_BAR_H, rx: 4 }, ["gantt-float"]));
  }

  // Duration bar (ES→EF)
  const bx = ganttX(item.es);
  const bw = Math.max(4, (item.ef - item.es) * GANTT_COL_W);
  const bar = makeSvgEl("rect", { x: bx, y: barY, width: bw, height: GANTT_BAR_H, rx: 4 });
  bar.classList.add("gantt-bar", item.critical ? "crit" : "noncrit");
  bar.dataset.id = item.id;
  g.appendChild(bar);

  const barLbl = makeSvgEl("text", {
    x: bx + bw / 2, y: barY + GANTT_BAR_H / 2,
  ...
  if (!isPertMode() && (item.ef - item.es) > 0) {
    const handle = makeSvgEl("rect", { x: bx + bw - 4, y: barY, width: 10, height: GANTT_BAR_H });
...
function ganttRedrawBars(svgEl, items) {
  items.forEach(item => {
    const row = svgEl.querySelector(`.gantt-row[data-id="${CSS.escape(item.id)}"]`);
    if (!row) return;
    ...
    if (lbl) { lbl.setAttribute("x", bx + bw / 2); lbl.textContent = bw >= 24 ? item.name : ""; }
...
function attachGanttDragHandlers(svgEl, items) {
  if (isPertMode()) {
    svgEl.querySelectorAll(".gantt-bar").forEach(bar =>
      bar.addEventListener("mousedown", e => { e.preventDefault(); showGanttPertWarning(); })
    );
    return;
  }
  svgEl.querySelectorAll(".gantt-handle").forEach(handle =>
    handle.addEventListener("mousedown", e => onGanttMouseDown(e, svgEl, items))
  );
}

function onGanttMouseDown(e, svgEl, items) {
  if (isPertMode()) { e.preventDefault(); showGanttPertWarning(); return; }
  if (dragState) return;
  e.preventDefault();
  e.stopPropagation();
  ...
  if (!item) return;
  ganttGhostData = [{ ...item }];
  dragState = { taskId, startX: e.clientX, startEf: item.ef, previewEf: item.ef, svgEl, items };
...
function onGanttMouseMove(e) {
  ...
  ganttRedrawBars(svgEl, items.map(t => t.id === taskId ? { ...t, ef: newEf } : t));
}
...
function scrollToFirstGanttTask() {
  ...
  if (firstBar) {
    mount.scrollTo({ left: Math.max(0, parseFloat(firstBar.getAttribute("x") || "0") - 50), behavior: "smooth" });
  }
}
...
async function exportGanttToPng() {
  ...
  // Fetch the gantt stylesheet and inject it into the SVG clone so canvas
  // rendering (isolated from the page stylesheet) gets the correct styles.
  ...
  if (ganttCssLink) {
    ...
    } catch (_) { /* proceed without injected styles on fetch failure */ }
  }
